...
```{r}
carsTwentyThreeOrMoreMpg <- subset(mtcars, mpg >= 23) #checks list of cars 23 or more mpg
carsTwentyThreeOrMoreMpg #variable selected./
```

3. How many cars get more than 23 mpg? Use one of the commands you
learned in the demystifying.R to answer this question.
```{r}
count <- dim(carsTwentyThreeOrMoreMpg) # count the dimensions 
count # 7 cars and 11 columns of data 
```
...
```{r}
subset(mtcars, mpg > 30 & hp > 100) #this acts as a boolean operator with the and method
```
...
```{r}
subset(mtcars, mpg < 14 | disp > 390) #this acts as a boolen operator with the or method
```
...
```{r}
lightCars <- subset(mtcars, wt < 2.0) #cars that weigh under 2000lbs
lightCars
```
...
Run the code below.
```{r}
mtcars$year <- 1974 #creates new column or variable
mtcars # this creates a new column of 1974 cars
```
...
```{r}
mtcars <- subset(mtcars, select = -year) #drops the column year in the dataframe
```
...
```{r}
mtcars$year <- c(1973, 1974) #this does a column years and offsets 73 and 74
mtcars
```
...
```{r}
mtcars <- subset(mtcars, select =-year) #drop the 73' 74' dataset
```
...
more code.
```{r}
mtcars$wt #select the weight column
cond <- mtcars$wt < 3 #create weight that is less than 3 call cond
cond #call cond 
mtcars$weight_class <- ifelse(cond, 'light', 'average') #create new column called weight_class all statements that are 3 TRUE == 'light', FALSE == 'Average'
mtcars$weight_class # calls the new column
cond <- mtcars$wt > 3.5 # remakes new condition 
mtcars$weight_class <- ifelse(cond, 'heavy', mtcars$weight_class) #if TRUE ='heave' if FALSE = keep variable in weight_class
mtcars$weight_class # calls the new column 
```
...
```{r}
carsMpgHorsePower <- subset(mtcars, mpg >= 30 | hp < 60) # Mpg >= 30 or horsepower < 60
carsMpgHorsePower
```
