...
colnames(dtdf) <- c("cmte_id", "cand_id", "canidate", "name","city", "state", "zip", "employer", 
                    "occupation", "amount", "date","receipt", "memo",
...
bsdf[nm] <- lapply(nm, function(x) geoloc[[x]][match(as.character(bsdf$city), 
                                                     as.character(geoloc$city))]) #create loc dataset for bernie
dtdf[nm] <- lapply(nm, function(x) geoloc[[x]][match(as.character(dtdf$city), 
...
largest_cities_dtdf <- subset(dtdf, city == c("BAKERSFIELD", "SAN JOSE", "LOS ANGELES",
                                              "SACRAMENTO","SAN DIEGO", "HUNTINGTON BEACH")) #sanders contributions in bigger cities
...
```
Within the summary you can see that largest city of contributors is Los Angeles. An overwhelming amount are Not Empoyed and most people made contributions around April of 2016. There are some strange things to poitn out like the minimum amount is listed as -10,500 dollars. This took me awhile to understand and with a little more investigating you can see in the receipt section there is a section called refunds were contributors may refund based on legalities of contributions in the state of california. 
...
ggplot(aes(x = amount, color = I("black"), fill = I("firebrick4")), data = merge_bs_dt_df_subset) + 
...
Sanders has a bit more contributions that are less then the fifty-dollar range peaking above 10 thousand, while Trump appears to have a larger count of amount around the nine-hundred dollar range. Let's investigate 
...
choose_city_df_sample <- subset(merge_bs_dt_df, 
                                merge_bs_dt_df$city %in% c("BAKERSFIELD", "SAN JOSE", "SAN FRANCISCO", "BERKELEY", "LOS ANGELES",
...
choose_occupation_df_sample <- subset(merge_bs_dt_df, 
                                      merge_bs_dt_df$occupation %in% c("NOT EMPLOYED", "RETIRED", "INFORMATION REQUESTED", 
                                                                                       "TEACHER", "ENGINEER"))
...
choose_name_df_sample <- subset(merge_bs_dt_df, 
                                merge_bs_dt_df$name %in% c("WEIL, MONIQUE", "MCLENNAN, MARLYN", "AUSLENDER, LEONARD", "ISERI, MARTIN", 
...
ggplot(aes(x = amount, color = I("black"), fill=  I('#F79420')), data = not_employed_df) +
...
#color plot 
ggplot(aes(x = amount, color = I("black"), fill = I('#099DD9')), data = largest_cont_df) + 
  geom_histogram(boundary = 0 ) + 
  labs(title = "Bernie V Trump Contributions", 
...
ggplot(merge_samp, aes(x = date, y = amount)) + 
  geom_line(aes(color= canidate), stat="identity", position = position_nudge()) + 
...
ggplot(data = choose_city_df_sample, aes(date, amount)) + 
  ...
  labs(titie = "Line Graph for Sanders V Trump", 
       ...
       caption = "source: .gov") + 
  facet_wrap(~canidate, ncol = 1) +   
  scale_color_brewer(type = "div", palette = "RdBu", guide = guide_legend(title = "Color", reverse = F, override.aes = list(alpha = 1, size = 2))) + 
...
ggplot(data = ca_county, aes(x = long, y = lat, group = group)) + #chgoose ca_county dataset 
  ...
  geom_jitter(data = sample_bernie, aes(x =lon, y = lat, group= NULL), color = "blue", alpha = 1) +  #choose bernie dataset 
  geom_jitter(data = sample_trump, aes(x = lon, y = lat, group = NULL), color = "red", alpha = 0.3) + #choose trump dataset
...
These last two graphs show the overall range of were people are contribution. As you have noticed in the map you see a lot of red within the San Diego region and more across the ocean. However, these regions 
to have some dots for Bernie. Purple represents counties and cities where you have equal amounts of of contributions. However, this is more indepth with a sample size of 50 contributors with the heat map. 
Within the heat map you see most of the areas are converging to purple with some leaning blue and other leaning more red. This gives us a good representation how the overall distrubtion of how voters were contributing in California. 
...
ggplot(aes(x = amount,color = I("black"), fill = canidate), data = merge_bs_dt_df_subset) + 
  geom_histogram(boundary = 0) + 
  labs(title = "Bernie v Trump Contributions Base 10",
       subtitle = "Count vs. Amount", 
       caption = ".gov") + 
...
row.names(cdf) <- cdf$name #make row into names
cdf_matrix <- data.matrix(cdf) #create the data into a matrix of numberes
cdf_heatmap <- heatmap(cdf_matrix, Rowv=NA, Colv=NA, 
