...
ggplot(data = ca_county, aes(x = long, y = lat, group = group)) + 
  ...
  geom_jitter(data = sample, aes(x =lon, y = lat, group= NULL), color = "blue", alpha = 1) + 
...
geoloc <- read.csv('California_geoLocation_new.csv', row.names=NULL)
...
nm <- c("lon", "lat")
bsdf[nm] <- lapply(nm, function(x) geoloc[[x]][match(as.character(bsdf$city), as.character(geoloc$city))])
...
set.seed(20000)
sample <- bsdf[sample(1:length(bsdf$amount), 3000), ]
ggplot(data = sample, aes(x=lon, y =lat)) +
  geom_polygon() + 
  geom_point(aes(color = amount))  
  
head(ca_county)
california_plot <- ggplot(data = ca_county, mapping = aes(x = long, y = lat, group = group)) + 
  coord_fixed(1.3) + 
  geom_polygon(color = "black", fill = "gray") 

b1 <- ggplot(data = ca_county, aes(x = long, y = lat, group = group)) + 
      geom_polygon() + 
      geom_path(aes(color = subregion), linestyle = 2) +
      geom_jitter(data = sample, aes(x =lon, y = lat, group= NULL), color = "red", alpha = 0.8) 
  
```
```{r echo=false, create a trump map}
# map 
counties <- map_data("county")
ca_county <-subset(counties, region == "california")
...
set.seed(20000)
sample_trump <- bsdf[sample(1:length(dtdf$amount), 3000), ]

t1 <- ggplot(data = ca_county, aes(x = long, y = lat, group = group)) + 
  geom_polygon() + 
  geom_path(aes(color = subregion), linestyle = 2) +
  geom_jitter(data = sample_trump, aes(x =lon, y = lat, group= NULL), color = "yellow", alpha = 0.8) 
grid.arrange(b1,t1, ncol = 1)
```
